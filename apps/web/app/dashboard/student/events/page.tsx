...
export default function EventsPage() {
  const { events, toggleEventParticipation, loading } = useStudentData();
    ...
    return (
      <DashboardLayout title="Campus Events" icon={<Sparkles className="h-6 w-6" />}>
        <div className="flex flex-col items-center justify-center min-h-[300px] gap-3">
          ...
          <p className="text-xs text-slate-400 font-semibold">Loading campus events...</p>
        </div>
      </DashboardLayout>
    );
  }
    ...
    <DashboardLayout title="Campus Events" icon={<Sparkles className="h-6 w-6" />}>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="p-6 bg-slate-50 dark:bg-slate-900/40 rounded-2xl border border-slate-100 dark:border-slate-900 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <Badge variant="primary" className="text-xs">Admin Controlled Registration</Badge>
        </div>
        ...
        {events.length === 0 ? (
          <div className="p-12 text-center border border-dashed border-slate-200 dark:border-slate-800 rounded-2xl bg-slate-50/20 dark:bg-slate-900/5">
            ...
            <p className="text-xs text-slate-450 mt-1">Check back later for newly published campus events.</p>
          </div>
        ) : (
              ...
              return (
                <Card key={evt.id} className="flex flex-col border-slate-100 overflow-hidden bg-white dark:bg-slate-950">
                  <CardHeader className="flex flex-row items-start justify-between pb-3">
                    ...
                    <Badge variant={getCategoryBadge(evt.category) as any} className="capitalize text-[9px]">
                      {evt.category}
                    </Badge>
                  </CardHeader>
                  ...
                  <CardContent className="flex-1 p-5 pt-0 space-y-4 text-xs flex flex-col justify-between">
                    ...
                    <div className="flex flex-col gap-2 pt-2">
                      ...
                      >
                        {evt.isParticipating ? (
                          <><Check className="h-3.5 w-3.5" /> Participating</>
                        ) : isRegistrationClosed ? (
                          'Registration Closed'
                        ) : noSeatsLeft ? (
                          'No Seats Left'
